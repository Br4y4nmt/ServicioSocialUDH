...
import {
  showTopSuccessToast,
  showTopWarningToast
} from "../../../hooks/alerts/useWelcomeToast";
...
const SeguimientoActividades = ({
      ...
      try {
        ...
        if (Array.isArray(data) && data.length > 0) {
          ...
          if (estadoPlan === 'aceptado') {
            Swal.fire(
              'Sin cartas',
              'No se encontraron cartas de aceptación del grupo.',
              'info'
            );
          }
        }
        ...
        Swal.fire(
          'Error',
          'No se pudo cargar las cartas del grupo.',
          'error'
        );
      }
    },
  ...
  const actividadPorId = useMemo(() => {
    ...
    actividadesSeguimiento.forEach((actividad) => {
      map.set(actividad.id, actividad);
    });

    return map;
  }, [actividadesSeguimiento]);

  const mostrarColumnaNueva = actividadesSeguimiento.some(
    (actividad) => actividad.correccion_habilitada
  );

  const handleEnviarEvidencia = useCallback(
    async (actividadId) => {

      if (cargandoEvidencia[actividadId]) {
        return;
      }

      setCargandoEvidencia((prev) => ({
        ...prev,
        [actividadId]: true
      }));
      ...
      try {
        ...
        if (actividad.correccion_habilitada) {
          ...
          if (!actividad.fecha_limite_reenvio) {
            await alertError(
              ...
              'No se encontró una fecha límite para reenviar esta evidencia.'
            );
        ...
        if (!actividad.correccion_habilitada) {
          ...
          if (!actividad.fecha_fin_primero) {
            await alertError(
              ...
              'Esta actividad no tiene una fecha permitida para subir evidencia.'
            );
          ...
          if (diferenciaEnDias > 10) {
            await alertError(
              'Fecha vencida',
              `La fecha permitida (${fechaPermitidaStr}) ya venció. No es posible subir la evidencia.`
            );
        ...
        if (!actividad.archivoTemporalEvidencia) {
          showTopWarningToast(
            'Falta evidencia',
            'Selecciona o sube una evidencia antes de enviar.'
          );
        ...
        formData.append(
          'evidencia',
          actividad.archivoTemporalEvidencia
        );
          ...
          prev.map((a) =>
            a.id === actividadId
              ? {
                  ...
                  fecha_fin:
                    res.data.fecha_fin || fechaHoy,

                  estado:
                    'pendiente',

                  archivoTemporalEvidencia:
                    null,
        ...
        if (eraCorreccion) {
          showTopSuccessToast(
            'Corrección enviada',
            'Tu evidencia corregida fue enviada correctamente y será revisada nuevamente.'
          );
        } else {
          showTopSuccessToast(
            'Evidencia enviada',
            'Tu evidencia fue enviada correctamente.'
          );
        }
  ...
  const handleSeleccionarEvidencia = useCallback(
    async (actividadId, index) => {
  ...
  const handleVerEvidencia = useCallback(
    (evidenciaFilename) => {

      setImagenModal(
        `${process.env.REACT_APP_API_URL}/uploads/evidencias/${evidenciaFilename}`
      );
  ...
  return (
    <div className="seguimiento-container">
                ...
                (item, index) => (
                  ...
                  <tr key={item.id}>
                    ...
                    <td
                      ...
                      onClick={() =>
                        handleVerDetalleActividad(item)
                      }
                      title="Ver detalles"
                    >
                      {item.actividad?.length > 10
                        ? item.actividad.slice(0, 10) + '...'
                        : item.actividad}
                    </td>

                    <td>
                      {item.justificacion?.length > 10
                        ? item.justificacion.slice(0, 10) + '...'
                        : item.justificacion}
                    </td>
                    ...
                    <td>
                      ...
                      {item.fecha_fin_primero ? (
                        ...
                        <span className="fecha-no-asignada">
                          No asignada
                        </span>
                    ...
                    {mostrarColumnaNueva && (
                      ...
                      >
                        {item.correccion_habilitada ? (
                          item.archivoTemporalEvidencia ? (
                            <div
                              title="Nueva evidencia seleccionada"
                              style={{
                                display: 'flex',
                                justifyContent: 'center'
                              }}
                    ...
                    >
                      ...
                      {item.correccion_habilitada ? (
                        ...
                        <button
                          onClick={() =>
                            handleEnviarEvidencia(item.id)
                          }
                          className="btn-enviar-evidenciasss"
                          title="Enviar evidencia corregida"
                          disabled={
      ...
      {todasAprobadas &&
        estadoSolicitudTermino === 'no_solicitada' && (
            ...
            <button
              ...
              onClick={async () => {
                ...
                try {
                  ...
                  setTimeout(
                    () =>
                      setEnviandoSolicitudTermino(false),
                    1000
                  );
                }
              }}
      ...
      {(
        estadoSolicitudTermino === 'solicitada' ||
        ...
        <div className="respuesta-asesor-card">
            ...
            <div className="respuesta-acciones">
              ...
              >
                {estadoSolicitudTermino
                  .charAt(0)
                  .toUpperCase() +
                  estadoSolicitudTermino.slice(1)}
              </button>
          ...
          {estadoSolicitudTermino === 'aprobada' ? (
            ...
            <p className="texto-cursiva">
              Se aprobó correctamente su solicitud.
              Puede ver y descargar su documento de término
              de actividades.
            </p>
          ) : (
            ...
            <p className="texto-cursiva">
              ...
              El docente revisará tu cronograma antes de
              aprobar la carta.
            </p>
      ...
      {estadoSolicitudTermino === 'aprobada' && (
        ...
        <div className="seccion-documentos-designacion">
          ...
          <div className="seccion-header">
            ...
            <h3>
              {cartasMiembros.length > 1
                ? 'Cartas de término del grupo'
                : 'Documentos de aprobación de actividades'}
            </h3>
            <div className="info-tooltip">
              <InfoTooltipIcon />
              <div className="tooltip-text">
                Aquí podrás visualizar las cartas de término
                generadas para todos los miembros del grupo.
              </div>
            </div>
          </div>
          ...
          {cartasMiembros.map((carta) => (
            ...
            <div
              key={carta.codigo_universitario}
