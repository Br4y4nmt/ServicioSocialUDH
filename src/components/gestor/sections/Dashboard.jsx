...
const toggleRegistro = async () => {
  ...
  const action = registroHabilitado ? 'deshabilitar' : 'habilitar';
  const result = await alertconfirmacion({
    title: `¿Estás seguro de ${action} el registro de estudiantes?`,
    text: registroHabilitado
      ? 'Esto impedirá que nuevos estudiantes puedan crear cuenta en el sistema.'
      : 'Esto permitirá que nuevos estudiantes puedan crear cuenta en el sistema.',
    icon: 'warning',
    confirmButtonText: registroHabilitado ? 'Sí, deshabilitar' : 'Sí, habilitar',
...
return (
  <Box sx={{ p: 3 }}>
    ...
    <Grid container spacing={2} sx={{ mb: 2 }}>
      <Grid xs={12} md={9} className="chart-grid-wide">
